...
RUN apt-get install --assume-yes libjpeg-dev zlib1g-dev gfortran libopenblas-dev liblapack-dev libatlas-base-dev libblas-dev  -y \
    && apt-get --assume-yes install python3.9 -y  \
    && apt-get --assume-yes install git -y  \
    ...
    && apt-get clean && pip3 cache purge && rm -rf /var/cache/*
