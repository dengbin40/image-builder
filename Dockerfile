...
RUN apt-get install --assume-yes --no-install-recommends apt-utils libjpeg-dev zlib1g-dev gfortran libopenblas-dev liblapack-dev libatlas-base-dev libblas-dev  -y \
    && apt-get --assume-yes --no-install-recommends install python3.9 -y  \
    && apt-get --assume-yes --no-install-recommends install git -y  \
    ...
    && apt-get clean && pip cache purge && rm -rf /var/cache/*
